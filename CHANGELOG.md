...
### Added
- Added a shared web novel source model and a standalone Kindle EPUB conversion pipeline, so site readers can hand normalized book data to EPUB generation without using the existing repair engine.
- Added ESJZone as the first source reader, including search parsing, detail metadata extraction, table-of-contents parsing, chapter content fetching, and image downloading.
- Added JSON-lines backend options for ESJZone conversion and search.
- Added a WinUI ESJZone page for detail URL conversion with popup web login, automatic Cookie capture, output directory selection, optional chapter limit, progress, and logs.
- Added validation for EPUBs generated by the web novel conversion pipeline.
- Added ESJZone chapter range selection, so inputs such as `1-10` generate only that inclusive chapter range.
- Added optional remembered ESJZone Cookie storage for the WinUI source page.

### Changed
- Changed the default ESJZone endpoint to `https://www.esjzone.cc/` while keeping `https://www.esjzone.one/` detail URLs supported.
- Changed ESJZone chapter limiting so leaving the chapter count blank means all chapters are fetched automatically.
- Changed generated ESJZone EPUB navigation to include only clickable chapter entries, skipping non-clickable volume headings from the source page.
- Changed generated web-novel styling to use Kindle-friendly generic font families: `sans-serif` for body text and `serif` for headings.
- Changed animated GIF/WebP assets in generated web-novel EPUBs to static JPEG first frames for better Kindle compatibility.
- Changed the generated book-info page so it no longer renders a visible `书籍信息` heading.
...
- Fixed generated web-novel XHTML documents so Sigil no longer has to repair missing DOCTYPE/html/head/body structure.
- Fixed ESJZone author metadata parsing so the EPUB creator uses the actual linked author instead of the `作者:` label.
- Fixed artificial ESJZone blank paragraphs so single spacer lines are removed while multi-line scene breaks are preserved.
- Fixed generated web-novel image handling so inline `data:image` and lazy-loaded image URLs are converted into EPUB resources.
- Fixed WinUI start actions so the button state paints before backend work starts.
- Fixed repair log flyouts to wrap and refresh live, and ESJZone conversion logs to stay scrolled to the latest line.

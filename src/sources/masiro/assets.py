"""Resource helpers for Masiro chapter content."""
...
def decode_data_image(value: str) -> tuple[bytes, str, str] | None:
...
def image_source(img: etree._Element) -> str:
    for attr in ("data-src", "data-original", "data-lazy-src", "src"):
        value = (img.get(attr) or "").strip()
        if value and "empty" not in value.lower() and "blank" not in value.lower():
            return value
    for attr in ("data-srcset", "srcset"):
        candidates = [part.strip() for part in (img.get(attr) or "").split(",") if part.strip()]
        if candidates:
            return candidates[-1].split()[0]
    return ""


def guess_image_type(url: str, data: bytes) -> tuple[str, str]:
